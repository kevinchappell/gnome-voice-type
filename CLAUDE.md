...
./dev.sh nested       # launch devkit GNOME Shell — REQUIRED for testing on Wayland (was --nested before Shell 49)
./dev.sh test         # devkit session with the extension auto-enabled
...
You cannot reload the extension into the running shell on Wayland (Shell 49+ disabled `Meta.restart()` and `ReloadExtension`) — always use `./dev.sh nested` (which now runs `gnome-shell --devkit`, since `--nested` was removed in Shell 49). On X11, `./dev.sh reload` works in-session.
